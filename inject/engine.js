...
function checkVerification(){
	if($('.pop-common-error:contains("verification")').length>0)
	{
		window.Engine.stop();
	}
}
function skiperror() {
	if (getStrMatch(String($("div.prt-error-infomation").find("p").html()), "エラーが発生しました。") ||$('.pop-rematch-fail').length>0) {
...
function turnWaitCancel() {
	...
	else if("Waiting for last turn to be processed" == $("#pop div.txt-popup-body").html()	&& $("#pop div.btn-usual-ok").attr("oshita") == undefined){
		$("#pop div.btn-usual-ok").attr("oshita","1");
		tap($("#pop div.btn-usual-ok"));
	}
}
...
window.Engine = {
	...
	start: function() {
		var self = this;
		var script = $('script:eq(1)')[0];
		var scriptText=script.innerText;
		var index=scriptText.indexOf("Game.lang");
		var language=scriptText.substring(index+13,index+15);
		/*$.getScript( url, function( data, textStatus ) {
  			window.scriptJS=data;
  			console.log( data );
	...
	heal: function(index) {
		...
		return this.waitOnce('.pop-raid-item.pop-show').then(function() {
			...
			if (greenPotionCount > 0) {
				...
				return this.waitOnce('div.prt-popup-header:contains("アイテムを使用")').then(function() {  //Use Item
	...
	new_battle: function() {
		...
		if ($(".pop-show .prt-popup-header").length > 0) {
			if($('div.prt-popup-header:contains("アイテムを使用")').length > 0 ||$('div.prt-popup-header:contains("Use Item")').length > 0){   // original is !$('div.prt-popup-header:contains("アイテムを使用")').length > 0
				...
				this.click(".pop-show .prt-popup-footer .btn-usual-cancel")
			}
		}
		...
		if ($(".btn-attack-start").hasClass("display-on")) {
			...
			for (var p = 0; p <= 3; p++) {//use-green-potion use-event-potion  use-event-revive
				if (this.isInjured50(p)) {  //original:&& (this.greenPotionCount > 0 || this.bluePotionCount > 0)   
					var test=$('.pop-usual .pop-raid-item');
					if($('.pop-usual .pop-raid-item').length>0)
					{
						var greenPotionCount = parseInt($('.having-num').eq(0).text());
						var bluePotionCount = parseInt($('.having-num').eq(1).text());
						var eventPotionCount = parseInt($('.having-num').eq(3).text() || 0);
						...
						this.click('.btn-temporary');
						//return; cannot return because it will cause another circle again to show the pop-usual and ignore what exists already
					}

					if(window.localStorage.getItem('use-green-potion') == 'true')
					{
						if (this.greenPotionCount > 0){
							this.click('.btn-temporary-small');
							this.sleep(1).then(function() {
								if($('div.txt-popup-body:contains("回復するメンバーをタッチしてください"):contains("キュアポーションを使用します")').length > 0 ||$('div.txt-popup-body:contains("Tap a member to heal"):contains("Use green potion")').length > 0){
									this.click('.btn-usual-use');
								 }
								this.click('.btn-command-character:eq(' + p + ')');
							}.bind(this));
							this.greenPotionCount--;
							return;
						}else if (this.bluePotionCount > 0) {
							this.click('.item-large.btn-temporary-large');
							this.sleep(1).then(function() {
								if($('div.txt-confirm:contains("オールポーションを使用します")').length > 0 || $('div.txt-confirm:contains("All members in battle will be healed")').length > 0){  
									this.click('.btn-usual-use');
								 }
							}.bind(this));
							this.bluePotionCount--;
					...
					if(window.localStorage.getItem('use-event-potion') == 'true'){
						//original:$('div.txt-popup-body:contains("使用するアイテムを選択してください"):contains("キュアポーション")').length > 0||$('div.txt-popup-body:contains("Tap the item to use"):contains("Green Potion")').length > 0
						if (this.greenPotionCount > 0){
							this.click('.btn-temporary-small');
							this.sleep(1).then(function() {
								this.click('.btn-command-character:eq(' + p + ')');
							...
							this.bluePotionCount--;
							return;
						}else if (this.eventPotionCount > 0) {
							this.click('.lis-item.btn-event-item[item-id="1"]');
							this.eventPotionCount--;
							return;
						}
			...
			//div.lis-summon[pos="' + (index + 1) + '"]
				if (window.localStorage.getItem('auto-summon') === 'true' &&this.summonCount<=1 ) {
					for (var i=0; i < 6; i++) {
						//var test=$('.lis-summon[pos="' + (i + 1) + '"]');
						//var test1=$('.lis-summon[pos="' + (i + 1) + '"]').hasClass("btn-summon-available");
						//var id=window.localStorage.getItem("summon-"+(i+1));
						//var result=window.localStorage.getItem("summon-"+(i+1)) == '1';

						if ($('.lis-summon[pos="' + (i + 1) + '"]').hasClass("btn-summon-available")&&
							window.localStorage.getItem("summon-"+(i+1)) == '1') {

							var id1=$(".btn-summon-use").attr("summon-id", $('.lis-summon[pos="' + (i + 1) + '"]').attr("summon-id"));   
						this.click('.btn-command-summon');
						this.summonCount++;
						//window.summonTurn=1;
						//summonCount=window.summonTurn;
						this.waitOnce('.summon-show').then(function() {
							this.sleep(0.5);
						}.bind(this)).then(function() {
							this.click('.lis-summon[pos="' + (i + 1) + '"]');
							this.waitUntilVisible('.btn-summon-use');
						}.bind(this)).then(function() {
							this.click('.btn-summon-use');
							this.sleep(0.5);
						}.bind(this));
						...
						return;
					}else if($('.lis-summon[pos="' + (i + 1) + '"]').hasClass("btn-summon-available")&&
						window.localStorage.getItem("summon-"+(i+1)) == '3' && 	bossMode == '2') {
						$(".btn-summon-use").attr("summon-id", $('.lis-summon[pos="' + (i + 1) + '"]').attr("summon-id"));
						this.click('.btn-command-summon');
						this.waitOnce('.summon-show').then(function() {
							this.sleep(0.5);
						}.bind(this)).then(function() {
						...
						return;
					}else if($('.lis-summon[pos="' + (i + 1) + '"]').hasClass("btn-summon-asvailable")&&
						...
						return;
						}else if($('.lis-summon[pos="' + (i + 1) + '"]').hasClass("btn-summon-available")&&     //".lis-summon:eq(" + i + ")"
			...
			if (ballteCount == ballteCountTotal) {
				...
				for (var i = 0; i <= 3; i++) {
					...
					for (var i = 0; i <= 3; i++) {
						for (var j = 0; j <= 3; j++) {
							...
							if (window.localStorage.getItem("skill-" + (i + 1) + "-" + (j + 1)) == '11' && $(".lis-ability:eq(" + c + ")").hasClass("btn-ability-available") && !$(".prt-command-chara:eq(" + i + ")").hasClass("ability-disable")) {
								this.click(".lis-ability:eq(" + c + ")");
								return
							}
							c++;
					...
					for (var i = 0; i <= 3; i++) {
						for (var j = 0; j <= 3; j++) {
							...
							if (window.localStorage.getItem("skill-" + (i + 1) + "-" + (j + 1)) == '12' && $(".lis-ability:eq(" + c + ")").hasClass("btn-ability-available") && !$(".prt-command-chara:eq(" + i + ")").hasClass("ability-disable")) {
								this.click(".lis-ability:eq(" + c + ")");
								return
							}
							c++;
				...
				this.click(".btn-attack-start");
				if (ballteCount == ballteCountTotal) {
					if (window.localStorage.getItem('FCrefresh') == 'true') {
				...
				/*if (ballteCount == ballteCountTotal) {
					if (window.localStorage.getItem('FCrefresh') == 'true') {
						var test=$(".prt-member .lis-character0 .prt-gauge-special-inner").attr('style') .split(':')[1].replace(/%;/, "");
						if ($(".prt-member .lis-character0 .prt-gauge-special-inner").attr('style') .split(':')[1].replace(/%;/, "") >= 100
							||$(".prt-member .lis-character1 .prt-gauge-special-inner").attr('style') .split(':')[1].replace(/%;/, "") >= 100
							||$(".prt-member .lis-character2 .prt-gauge-special-inner").attr('style') .split(':')[1].replace(/%;/, "") >= 100
							||$(".prt-member .lis-character3 .prt-gauge-special-inner").attr('style') .split(':')[1].replace(/%;/, "") >= 100
							) {
							setTimeout(function() {
								window.location.reload();
								this.Engine.start()
