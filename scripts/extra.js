		...
		'_handle_quest/assist': function() {
			...
			if (window.localStorage.getItem('autoSeachEx') === 'true') {
				...
				if (window.localStorage.getItem("ptSearchType") == '2') {
					...
					var searchContent=searchName.split(" ");
					var lvl=searchContent[0];
					console.log(lvl);
					var requesturl = "http://realtime.search.yahoo.co.jp/search?p=参加者募集+"+lvl+"+" + searchContent[1]+ "&ei=UTF-8";
				...
				function handleData(text){
					console.log("你好1");
					str=text;
					var tableIndex=str.indexOf("id=\"TSm\"");
					//console.log(tableIndex);
					var	table=str.substring(table);

					var latestTwitterIndex=table.indexOf("<div class=\"cnt cf\"");
					var content=table.substring(latestTwitterIndex);

					var latestContentIndex=content.indexOf("h2>");
					var endingLatestTwitterIndex=content.indexOf("<em>");
					var latestTwitter=content.substring(latestContentIndex,endingLatestTwitterIndex);
					//console.log(latestTwitter);

					var getFirst=latestTwitter.indexOf(":参戦ID");
					//console.log(getFirst);
					var RaidId=latestTwitter.substring(3,getFirst);
					re=RaidId;
					console.log("参战id========" + re);
					
					var bbf = setInterval(function() {
						if (document.querySelector('.frm-battle-key').type == 'password'||document.querySelector('.frm-battle-key').type == 'text') {
	...
	'_handle_coopraid/room': function() {
		...
		//var test=$(".pop-usual.pop-stamina.pop-show");
		//var length=$(".pop-usual.pop-stamina.pop-show").length;
		if($(".pop-usual.pop-stamina.pop-show").length<=0)
		{
			var room = setInterval(function(){
				...
				{
					/*var maxAP = $('.use-item-num :last').val(); 
					$(".prt-item-disp:eq(0) select.use-item-num").val(maxAP/2-1); 
					$(".prt-item-disp:eq(1) select.use-item-num").val(maxAP-1);*/
					clearInterval(room);
				}
			},500);//the original time is 1000 but 500 is much better 
		}
		else
		{
			clearInterval(room);
		...
		if (window.localStorage.getItem('master') == 'true') {
			var startRoom = setInterval(function(){
				if ($('.btn-use-full').length > 0) {
					...
					//var second=$('.prt-item-disp:eq(1)').find('div.btn-select-num');
					var currentAp=$('.txt-stamina-value').attr("title").split("/");;
					var maxAP = $('.use-item-num :last').val(); 
					$(".prt-item-disp:eq(0) select.use-item-num").val(maxAP/2-1); 
					$(".prt-item-disp:eq(1) select.use-item-num").val(maxAP-1);
					//var test=$("div.prt-item-disp:eq(1) div.prt-use-button div.btn-use-full");
					$('div.prt-item-disp:eq(1) div.prt-use-button div.btn-use-full').attr("data-use-num", maxAP-1);
					var perHalfAp=$('div.prt-item-disp:eq(1) div.prt-use-button div.btn-use-full').attr("data-recovery-point");
					var finalAp=(maxAP-1)*perHalfAp+parseInt(currentAp[0]);
					$('div.prt-item-disp:eq(1) div.prt-use-button div.btn-use-full').attr("data-after-sp", (maxAP-1)*perHalfAp+parseInt(currentAp[0]));
					self.click('div.prt-item-disp:eq(1) div.prt-use-button div.btn-use-full');
					//pop-usual pop-result start-coopraid pop-show
					self.waitOnce('.pop-usual .pop-result .start-coopraid .pop-show').then(function() {
						self.click('.btn-usual-ok');
						self.sleep(1).then(function() {
							if (window.location.hash.indexOf('coopraid') >= 0) {
								self['_handle_coopraid/room']();
							}
						}.bind(self));
					}.bind(self));
				...
				if(!$(".btn-quest-start").length > 0 && $(".btn-usual-ok").length < 1){
					...
					var id,cost;
					if(window.localStorage.getItem('ptcoopraid')=='999999'){
						var fri=window.localStorage.getItem('coopFriSelect');		
						var sec=window.localStorage.getItem('coopSecSelect');	
						var thr=window.localStorage.getItem('coopThrSelect');	
						if(fri=='HARD'){
							if(sec=="1"){id=600911+((parseInt(thr)-1)*10);cost="20"}
							if(sec=="2"){id=601011+((parseInt(thr)-1)*10);cost="20"}
							if(sec=="3"){id=601111+((parseInt(thr)-1)*10);cost="20"}
							if(sec=="4"){id=601211+((parseInt(thr)-1)*10);cost="20"}
							if(sec=="5"){id=601311+((parseInt(thr)-1)*10);cost="24"}
							if(sec=="6"){id=601411+((parseInt(thr)-1)*10);cost="24"}
							if(sec=="7"){id=601511+((parseInt(thr)-1)*10);cost="24"}
							if(sec=="8"){id=601611+((parseInt(thr)-1)*10);cost="24"}
						}else{
							if(sec=="1"){
								if(thr>"5") alert("没有这关！")
									id=601711+((parseInt(thr)-1)*10);
								cost="36";				
							}
							if(sec=="2"){
								if(thr>"4") alert("没有这关！")
									id=601811+((parseInt(thr)-1)*10);
								cost="40";				
							}
							if(sec=="3"){
								if(thr>"4") alert("没有这关！")
									id=601911+((parseInt(thr)-1)*10);
								cost="40";				
							}
							if(sec=="4"){
								if(thr>"4") alert("没有这关！")
									id=602011+((parseInt(thr)-1)*10);
								cost="40";				
							}							
							if(sec>"4"){
								alert("没有这关！")
						...
						id= window.localStorage.getItem('ptcoopraid');
					...
					setBtnElm.setAttribute("data-quest-id", id.toString()), 
					setBtnElm.setAttribute("data-ap", cost),
