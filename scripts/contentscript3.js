	...
	'_handle_item': function() {
		...
		this.waitUntilVisible('.lis-item').then(function() {
			...
			if (this.restoreFullAP()) {
				this.click('div.btn-item-tabs.items');
				this.waitOnce('div.btn-item-tabs.items.active').then(function(){
					this.click('.prt-item-inner .lis-item.se[data-index=0]');
				}.bind(this));
				clearTimeout(getTimeOut);
			} else if(this.restoreHalfAp()){
				this.click('div.btn-item-tabs.items');
				this.waitOnce('div.btn-item-tabs.items.active').then(function(){
					this.click('.prt-item-inner .lis-item.se[data-index=1]');
				}.bind(this));
				clearTimeout(getTimeOut);
				//this.click('.prt-item-inner .lis-item.se[data-index=1]');
			}
			...
			if(this.fullfillAPOnce()==true)
			{
				var maxAP = $('.num-set.use-item-num :last').val(); 
				$('.pop-normal .num-set').trigger("change");
				this.Tapchange_('.pop-normal .num-set');
				this.Tapchange_('.pop-normal .num-set option[value="'+(maxAP-1)+'"]');
				//$('.num-set.use-item-num option[value="'+(maxAP-1)+'"]').addClass("selected");
				var selector=document.querySelector(".num-set.use-item-num");  
				selector.value=maxAP-1;
				var b=$(".num-set").children(":selected");
				$(".pop-normal .num-set").find("option:contains("+(maxAP-1)+")").attr("selected",true);
				//var test=$(".num-set.use-item-num").find("option:selected").val();
				$('.pop-normal .num-set').attr('value','15');
				var before=$('div.txt-after-num span.before:eq(0)').text();
				var halfAp=$('div.txt-after-num span.after-point').text()-before;
				$('div.txt-after-num span.after-point').text(parseInt(halfAp*(maxAP-1))+parseInt(before));
				var currentHalfAp=$('div.txt-after-num span.before:eq(1)').text();
				$('div.txt-after-num span.after-num').text(parseInt(currentHalfAp)-(maxAP-1));
			//$('.num-set.use-item-num').val(maxAP-1); 
			//this.click('.num-set.use-item-num option[value="'+(maxAP-1)+'"]');
...
restoreHalfAp: function(){
	return window.localStorage.getItem('use-semi') === 'true';
},	
restoreFullAP: function() {
	return window.localStorage.getItem('use-full') === 'true';
},
fullfillAPOnce: function(){
	return window.localStorage.getItem('fullfilledOnce') === 'true';
},
...
Tapchange_: function(selector){
	console.log('changing ' + selector);
	this.sleep(1 + Math.random()).then(function() {
		window.localStorage.setItem('change-selector', selector);
		this.inject('inject/change_.js')
	}.bind(this));
},
load: function(hash) {
...
'_handle_quest/stage': function() {
	if($('.btn-command-forward').length>0)
	{
		...
		//var test=$("#pop");
		var skip=this.waitOnce(".pop-skip-result").then(function(){
			this.click('div.pop-skip-result div.prt-popup-footer div.btn-usual-ok');
			this.sleep(1).then(function() {
				if (window.location.hash.indexOf('stage') >= 0) {
					this['_handle_quest/stage']();
				}
		...
		/*this.waitOnce(".pop-usual .pop-skip-result .pop-show").then(function(){
			this.click('div.pop-skip-result div.prt-popup-footer div.btn-usual-ok');
		}.bind(this));*/
	}
},
...
/*
    ...
    */
    ...
    if ($('.icon-supporter-type-f').hasClass("selected")==false)
    {
    	...
    	/* 
    	通过维拉插件没有优先选择最爱选项卡。则进行召唤石效果量比较
    	if there's not favorite summons firstly selected, then we compare all the effects of the summons.
    		*/
    	button = '[data-supporter-user-id="' + candidates[0].id + '"][data-attribute="' + set_attribute + '"]';
    }
    else{
    	/* 
    	通过维拉插件优先选择最爱选项卡 但是没有最爱召唤则怎么办。
    	if there's favorite summons firstly selected, then we just need the first selected favorite summon.
    	but if the first summon is not the favorite, we just go back to choose the water attribute summons.
    */
    if(candidates[0].attribute!=set_attribute){
    	button = '[data-supporter-user-id="' + candidates[0].id + '"][data-attribute="' + set_attribute + '"]';
    }
    else{
    	button = '[data-supporter-user-id="' + candidates[0].id + '"][data-attribute="' + candidates[0].attribute + '"]';
    }
...
return this.sleep(1).then(function() {
	...
	'_handle_quest/supporter': function() {
		...
		var arr= window.localStorage.getItem('misson-database');
		var self = this;
	...
	'_handle_raid': function() {
		Promise.race([this.waitUntilVisible('.prt-start-direction'), this.sleep(1)]).then(function() {
	...
	'_handle_result_multi': function() {
		...
		if (window.localStorage.getItem('master') == 'true') {
			var bbc = setInterval(function() {
				if ($('.btn-control').length > 0) { //btn-retry cnt-quest
			...
			/*waitOnce('pop-usual pop-exp pop-show').then(function(){
				location.href = "http://"+window.location.host+"/#coopraid";
			})*/
		}else if(window.localStorage.getItem('coopraid') == 'true')
		{
			var jump=this.waitOnce('.pop-usual.pop-exp.pop-show').then(function(){
				location.href = "http://"+window.location.host+"/#coopraid";
				this.sleep(1).then(function() {
					if (window.location.hash.indexOf('result_multi') >= 0) {
						this['_handle_result_multi']();
					}
				}.bind(this));
			}.bind(this));
...
var timer=setInterval(function(){
	...
	if (misson== undefined||misson=='{}'||window.localStorage.getItem('enabled')=='false') {
		return;
	}else{
		misson = JSON.parse(misson);
		for (var id in misson) {
			var timeStart = misson[id].start;
			var timeEnd = misson[id].end;
			var startHappyHourD = new Date();
			if(timeStart == null || timeStart == undefined || timeStart == ''||timeEnd == null || timeEnd == undefined || timeEnd == '')
				console.log("no starting tiime and ending time.");
			else
			{
				startHappyHourD.setHours(timeStart.substring(0, 2),timeStart.substring(2, 4),0);
				var endHappyHourD = new Date();
				...
				var currentD = new Date();
				if(currentD >= startHappyHourD && currentD < endHappyHourD ){
					window.localStorage.setItem('quest-id',misson[id].questId);
					window.localStorage.setItem('timeStart',timeStart);
					window.localStorage.setItem('timeEnd',timeEnd);
					chrome.extension.sendRequest({cmd:'setLoacl',questId:misson[id].questId,startTime:timeStart,endTime:timeEnd}, function(response) {
						console.log(response);
					});
				}else{
					chrome.extension.sendRequest({cmd:'clearlocal'}, function(response) {
						console.log(response);
					});
				}	
